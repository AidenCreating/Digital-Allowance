...
function render() {
  const showParent = currentRole === "parent" && !isAdmin;
  const showAdmin = currentRole === "parent" && isAdmin;
  const showChild = currentRole === "child";

  el.authSection.classList.toggle("hidden", showParent || showChild || showAdmin);
  el.parentPortal.classList.toggle("hidden", !showParent);
  el.adminPortal.classList.toggle("hidden", !showAdmin);
...
function renderAdminPanel() {
  ...
  if (!adminParentsTable || !adminChildrenTable) return;
