...
function onCreateChild(event) {
  ...
  const child = createChild(name, pin, 0, currentParentId);
...
function onChildListAction(event) {
  ...
  const child = state.users.children.find((entry) => entry.id === childId && entry.parentId === currentParentId);
  if (!child) return;
...
function onParentPurchaseAction(event) {
  ...
  const success = undoPurchase(childId, purchaseId, reason, currentParentId);
  if (!success) return;
...
function undoPurchase(childId, purchaseId, reason, parentId = "") {
  const child = state.users.children.find(
    (entry) => entry.id === childId && (!parentId || entry.parentId === parentId)
  );
  if (!child) {
...
function getSelectedChildForParent() {
  return (
    state.users.children.find(
      (child) => child.id === state.selectedParentChildId && child.parentId === currentParentId
    ) || null
  );
}
...
function createChild(name, pin, startingBalance = 0, parentId = "") {
  return {
    id: crypto.randomUUID(),
    name,
    pin,
...
function normalizeLoadedState(parsed) {
  ...
  const fallbackParentId = merged.users.parents[0]?.id || "";
  merged.users.children = merged.users.children
    .map((child) => normalizeChild(child, fallbackParentId))
...
function normalizeChild(child, fallbackParentId = "") {
  return {
    id: child.id || crypto.randomUUID(),
    name: String(child.name || ""),
    pin: String(child.pin || ""),
    parentId: String(child.parentId || fallbackParentId),
...
function getChildrenForParent(parentId) {
  if (!parentId) return [];
  return state.users.children.filter((child) => child.parentId === parentId);
}
