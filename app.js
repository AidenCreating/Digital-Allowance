...
function onAddInventoryItem(event) {
  ...
  child.inventory.push({ id: crypto.randomUUID(), name, price, stock });
  el.itemName.value = "";
  el.itemPrice.value = "";
  el.itemStock.value = "";
...
function onInventoryAction(event) {
  ...
  const item = child.inventory.find((entry) => entry.id === id);
  if (!item) return;

  if (action === "remove") {
    child.inventory = child.inventory.filter((entry) => entry.id !== id);
...
function onCartAction(event) {
  ...
  if (action === "plus") {
    const inventoryItem = child.inventory.find((inv) => inv.id === id);
...
function onCheckout() {
  ...
  for (const cartItem of child.cart) {
    const invItem = child.inventory.find((item) => item.id === cartItem.id);
...
function undoPurchase(childId, purchaseId, reason, parentPin = "") {
  ...
  for (const lineItem of purchase.lineItems) {
    const existing = child.inventory.find((item) => item.id === lineItem.id);
    if (existing) {
...
function renderShop(child) {
  const availableItems = child.inventory.filter((item) => item.stock > 0);
...
function migrateLegacyState() {
  ...
  try {
    ...
    migratedChild.inventory = Array.isArray(legacy.inventory) ? legacy.inventory : [];
